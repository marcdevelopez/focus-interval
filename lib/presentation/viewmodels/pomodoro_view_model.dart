...
class PomodoroViewModel extends Notifier<PomodoroState> {
  ...
  bool get _sessionMissingWhileRunning =>
      ref.read(sessionSyncServiceProvider).holdActive;
  ...
  PomodoroState build() {
    ...
    ref.listen<SessionSyncState>(
      sessionSyncServiceProvider,
      (prev, next) => _onSyncStateChanged(prev, next),
    );
    ...
    ref.onDispose(() {
      _sub?.cancel();
      // Do NOT detach SessionSyncService here — the stream must survive VM
      // rebuild to maintain session continuity across Riverpod invalidations
      // (AP-1 anti-pattern). SSS.detach() is called only on mode switch.
      _mirrorTimer?.cancel();
  ...
  bool _canReconcileOwnerTimelineBeforePublish() {
    final group = _currentGroup;
    if (group == null || group.status != TaskRunStatus.running) return false;
    final appMode = ref.read(appModeProvider);
    if (appMode != AppMode.account) return !isMirrorMode;
    final active = activeSessionForCurrentGroup;
    if (active != null) {
      return active.ownerDeviceId == _deviceInfo.deviceId;
    }
    if (_sessionMissingWhileRunning) {
      return _canPublishHeartbeatWhileSyncing();
    }
    if (!_machine.state.status.isActiveExecution) return false;
    final initiator = group.scheduledByDeviceId;
    return initiator == null || initiator == _deviceInfo.deviceId;
  }

  bool _reconcileOwnerTimelineBeforePublish(
    DateTime now, {
    required String reason,
  }) {
    if (!_canReconcileOwnerTimelineBeforePublish()) return false;
    final reconciled = _applyGroupTimelineProjection(
      now,
      skipControlsCheck: true,
    );
    if (!reconciled) return false;
    _bumpSessionRevision();
    if (kDebugMode) {
      debugPrint(
        '[ActiveSession] Reconciled owner timeline before publish '
        'reason=$reason '
        'group=${_currentGroup?.id ?? 'n/a'} '
        'taskIndex=$_currentTaskIndex '
        'status=${_machine.state.status.name} '
        'remaining=${_machine.state.remainingSeconds}',
      );
    }
    return true;
  }

  void _publishCurrentSession({DateTime? now, String reason = 'publish'}) {
    ...
    if (_isRunning(session.status) &&
        ...
        _currentGroup?.status == TaskRunStatus.running) {
      if (kDebugMode) {
        debugPrint(
          '[ActiveSession] Suppressed impossible publish '
          '(running+remaining=0) '
          'reason=$reason '
          'group=${session.groupId ?? 'n/a'} '
  ...
  void _subscribeToRemoteSession({required String reason}) {
    // Session stream is now owned by SessionSyncService. Attach is idempotent.
    final groupId = _currentGroup?.id;
    if (groupId == null) return;
    if (kDebugMode) {
      // Log close for the prior subscription slot (SSS handles actual lifecycle).
      debugPrint('[SessionSub] close vmToken=$_vmToken reason=$reason');
      debugPrint(
        '[SessionSub] open (→SSS) vmToken=$_vmToken groupId=$groupId reason=$reason',
      );
    }
    ref.read(sessionSyncServiceProvider.notifier).attach(groupId);
  }

  // _onSessionMissingLatchDebounced removed — hold/latch is now owned by
  // SessionSyncService. _onHoldStarted handles the equivalent VM side-effects.
  ...
  /// Called when SessionSyncService state changes.
  /// Routes session ingestion and hold lifecycle side-effects to VM handlers.
  void _onSyncStateChanged(SessionSyncState? prev, SessionSyncState next) {
    final prevSession = prev?.latestSession;
  ...
  bool _shouldApplySessionTimeline(
    PomodoroSession session, {
    PomodoroSession? previousSession,
  }) {
    // Owner handoff across devices can temporarily regress lastUpdatedAt
    // due to clock skew. Apply that first snapshot to rebase safely.
    if (previousSession != null &&
        previousSession.ownerDeviceId != session.ownerDeviceId) {
      return true;
    }
  ...
  void _updateMirrorStateFromSession(PomodoroSession session) {
    ...
    final resolvedNow = projectionNow ?? DateTime.now();
    var projected = _projectStateFromSession(
      session,
      projectionNow: projectionNow,
    );
    if (session.status.isActiveExecution &&
        projected.status == PomodoroStatus.finished) {
      final group = _currentGroup;
      if (group != null && group.status == TaskRunStatus.running) {
        final fallback = _projectFromGroupTimelineWithPauseOffset(
          group,
          resolvedNow,
        );
        if (fallback != null && fallback.state.status.isActiveExecution) {
          _currentTaskIndex = fallback.taskIndex;
          _currentItem = _resolveTaskItem(group, fallback.taskIndex);
          _currentTaskStartedAt = fallback.taskStartedAt;
          if (_currentItem != null) {
            configureFromItem(_currentItem!);
            projected = fallback.state;
            if (kDebugMode) {
              debugPrint(
                '[ActiveSession] Mirror projection fallback '
                '(prevent finished overshoot) '
                'group=${group.id} '
                'taskIndex=${fallback.taskIndex} '
                'status=${projected.status.name} '
                'remaining=${projected.remainingSeconds}',
              );
            }
          }
        }
    ...
    _applyProjectedState(projected, now: resolvedNow, syncMachine: false);
  }
  ...
  void handleAppResumed() {
    ...
    if (appMode == AppMode.account) {
      final localNow = DateTime.now();
      if (_reconcileOwnerTimelineBeforePublish(localNow, reason: 'resume')) {
        _publishCurrentSession(
          now: _serverNowFromOffset(localNow: localNow) ?? localNow,
          reason: 'resume-reconcile',
        );
      }
  ...
  PomodoroSession _repairInconsistentSessionCursor(
    ...
    final sessionPhaseStartedAt = session.phaseStartedAt;
    final activeZeroRemainingStale =
        session.status.isActiveExecution &&
        session.remainingSeconds <= 0 &&
        session.phase != null &&
        sessionPhaseStartedAt != null &&
        session.phaseDurationSeconds > 0 &&
        now.difference(sessionPhaseStartedAt).inSeconds >
            session.phaseDurationSeconds;
    final needsRepair =
        inactiveWhileGroupRunning ||
        activeZeroRemainingStale ||
        outOfRangePomodoro ||
    ...
    var targetState = _stateFromSession(
      session,
      remaining: session.remainingSeconds,
    );
    ...
    if (inactiveWhileGroupRunning ||
      ...
      if (projection != null) {
        ...
        return session;
      } else if (activeZeroRemainingStale) {
        return session;
      } else {
        final clampedPomodoro = currentPomodoro.clamp(
          1,
          canonicalTotalPomodoros,
        );
        ...
        final boundedRemaining = session.remainingSeconds.clamp(
          0,
          phaseDuration,
        );
    ...
    final runningOrPaused =
        _isRunning(targetState.status) ||
        targetState.status == PomodoroStatus.paused;
    DateTime? phaseStartedAt = session.phaseStartedAt;
    if (runningOrPaused && targetState.phase != null) {
      final elapsed = (targetState.totalSeconds - targetState.remainingSeconds)
          .clamp(0, targetState.totalSeconds);
    ...
    final repaired = PomodoroSession(
      ...
      pausedAt: targetState.status == PomodoroStatus.paused
          ? session.pausedAt
          : null,
  ...
  PomodoroSession _buildOwnedRecoveredSession(
    ...
    return PomodoroSession(
      ...
      pausedAt: session.status == PomodoroStatus.paused
          ? session.pausedAt
          : null,
      lastUpdatedAt: now,
