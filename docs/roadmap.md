...
- Phase 10 — BUG-016 follow-up: Task Editor must use preview-first editing for **Task weight (%)** and **Total pomodoros** with explicit two-mode selector (**Fixed total** default / **Flexible total**) and apply-or-cancel semantics (docs-first decision approved 27/03/2026). **Patch 1** (correctness: baseline freeze + blur/save sync fix, no UI changes) — **Closed/OK on 28/03/2026** (`8bad479`, owner-run packet PASS + local gate PASS). **Patch 2** (preview UX) — blocked on 9 open UX decisions (a–i: mode selector widget, preview content, Apply/Cancel placement, preview trigger, mode-switch behavior, snackbar fate, preview visual form, Cancel semantics, mode scope); see `docs/bugs/bug_log.md` BUG-016 for the full list.
