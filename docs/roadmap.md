...
- Phase 10 — BUG-016 follow-up: Task Editor must use preview-first editing for **Task weight (%)** and **Total pomodoros** with explicit two-mode selector (**Fixed total** default / **Flexible total**) and apply-or-cancel semantics (docs-first decision approved 27/03/2026). **Patch 1** (correctness: baseline freeze + blur/save sync fix, no UI changes) — **Closed/OK on 28/03/2026** (`8bad479`, owner-run packet PASS + local gate PASS). **Patch 2** (preview UX) — UX decisions closed 28/03/2026 and implementation landed on branch `fix/bug016-weight-edit-preview-modes` on 28/03/2026; local gate PASS (`flutter analyze`, targeted tests), device validation pending. Key decisions: segmented control Fixed/Flexible inside sheet, inline business-rule explanation per selected mode, compact non-redundant status messaging (green exact state; orange warning only after interaction), live preview, fields become tap targets, full-screen opaque preview surface, `Back = Cancel` semantics, Apply = local draft only, Save = persist, new ViewModel method `redistributeTotalPomodoros`, Flexible `%` search without hard cap, and continuous-time caution/reminder levels (`Unusual`/`Superhuman`/`Machine-level`) shown inline in preview and as `Total time` companion chips in Task List/Groups Hub.
