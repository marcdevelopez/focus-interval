...
- [ ] ID: `P0-F25-001` | Type: bug | Priority: P0 | Status: In validation | Source: `docs/bugs/validation_fix_2026_03_05/quick_pass_checklist.md:34` | Item: Local -> Account without false overlaps; ownership request delivered (Fix 25). | closed_commit_hash: `-` | closed_commit_message: `-` | evidence: Re-validation run 2026-03-16 (iOS iPhone17Pro + Chrome) confirmed blockers BUG-F25-A/B/C. Code fixes implemented on branch `fix-f25-transaction-order-and-owner-dialog`; next step is exact repro re-run on devices before closure.
- [ ] ID: `BUG-F25-A` | Type: bug | Priority: P0 | Status: In validation | Source: `docs/bugs/bug_log.md` (BUG-F25-A) | Item: Firestore transaction read-write ordering violation in requestLateStartOwnership — reads must precede all writes; current per-group loop interleaves read-write causing _commands.isEmpty assertion on groups.length > 1. Fix: split into Phase 1 (all reads) + Phase 2 (all writes) in firestore_task_run_group_repository.dart:292. | closed_commit_hash: `-` | closed_commit_message: `-` | evidence: Code refactor applied in `lib/data/repositories/firestore_task_run_group_repository.dart`; analyzer + mandatory suite PASS. Pending device re-validation of ownership request delivery.
- [ ] ID: `BUG-F25-B` | Type: bug | Priority: P0 | Status: In validation | Source: `docs/bugs/bug_log.md` (BUG-F25-B) | Item: _showOwnerResolvedDialog OK button captures outer BuildContext; if state disposes before tap, Navigator.of(context) throws. Fix: capture Navigator before await showDialog in late_start_overlap_queue_screen.dart:549. | closed_commit_hash: `-` | closed_commit_message: `-` | evidence: Dialog callback now uses captured root navigator in `lib/presentation/screens/late_start_overlap_queue_screen.dart`; analyzer + mandatory suite PASS. Pending device re-validation for non-crashing dismiss flow.
- [ ] ID: `BUG-F25-C` | Type: bug | Priority: P1 | Status: In validation | Source: `docs/bugs/bug_log.md` (BUG-F25-C) | Item: "Owner resolved" modal shown on owner device after conflict resolution because ownerDeviceId is cleared post-resolve → isOwner evaluates false → mirror-only dialog fires. Fix: add _resolved flag in _LateStartOverlapQueueScreenState; gate condition at late_start_overlap_queue_screen.dart:176 with && !_resolved. | closed_commit_hash: `-` | closed_commit_message: `-` | evidence: `_resolved` guard implemented in `lib/presentation/screens/late_start_overlap_queue_screen.dart`; analyzer + mandatory suite PASS. Pending device re-validation that owner no longer sees mirror-only dialog.
