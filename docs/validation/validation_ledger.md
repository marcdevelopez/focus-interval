...
- [ ] ID: `P0-F26-005` | Type: bug | Priority: P0 | Status: In validation | Source: `docs/specs.md:2282`, `docs/roadmap.md:240`, `docs/bugs/validation_fix_2026_03_07-01/quick_pass_checklist.md:418` | Item: Fix 26 Phase 6 runtime hardening — B1 keepAlive grace window in `PomodoroViewModel` + B2 VM-disposed auto-open recovery refresh in `ActiveSessionAutoOpener`; must pass exact repro and regression smoke on devices. | closed_commit_hash: `-` | closed_commit_message: `-` | evidence: Implemented on `2fc65e4`; local analyze and `[PHASE6]` smoke tests pass; waiting for device packet closure.
