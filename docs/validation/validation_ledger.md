...
- [ ] ID: `P0-F26-006` | Type: refactor | Priority: P0 | Status: In progress | Source: `docs/specs.md:2376`, `docs/roadmap.md:266`, `docs/dev_log.md` Block 580 | Item: Fix 26 rewrite contract + Stage A runtime — migrate countdown continuity to app-scope non-autoDispose `TimerService` while preserving legacy ownership/handoff paths; stream-null hold must degrade sync only (no timer freeze). | closed_commit_hash: `-` | closed_commit_message: `-` | evidence: Contract finalized in `docs/specs.md` section 10.4.10; `[REWRITE-CORE]` baseline (`11d3866`) was 1 pass / 4 fail. Stage A partial runtime wiring now implemented: `[REWRITE-CORE]` Invariants 1 and 2 PASS; Invariants 3/4/5 remain contract-gate pending. Regression smoke PASS on `[PHASE*]` subsets and pause/overlay suites.
