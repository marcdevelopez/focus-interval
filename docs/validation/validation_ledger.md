...
- [ ] ID: `P0-F26-006` | Type: refactor | Priority: P0 | Status: In progress | Source: `docs/specs.md:2376`, `docs/roadmap.md:266`, `docs/dev_log.md` Block 579 | Item: Fix 26 rewrite contract (docs-first) — define and approve architecture invariants before tests/runtime edits: app-scope non-autoDispose `TimerService` model, exact stream-null UX thresholds, ownership stale timeout policy, staged dual-path cutover strategy, `TimerRuntimeState` minimal field contract, `SessionSyncService` API contract, and `PomodoroViewModel` adapter contract. | closed_commit_hash: `-` | closed_commit_message: `-` | evidence: Contract refined in `docs/specs.md` section 10.4.10 with concrete interfaces; `[REWRITE-CORE]` baseline tests added in `test/presentation/viewmodels/pomodoro_view_model_session_gap_test.dart` and executed (2026-03-14): 1 pass / 4 fail (red-first baseline, no runtime implementation yet).
