...
- [ ] ID: `P0-F26-003` | Type: bug | Priority: P0 | Status: In validation | Source: `docs/bugs/validation_fix_2026_03_07-01/quick_pass_checklist.md:195`, `docs/bugs/validation_fix_2026_03_07-01/quick_pass_checklist.md:206`, `docs/bugs/validation_fix_2026_03_07-01/plan_validacion_rapida_fix.md:377` | Item: Reopen/owner-switch must not persist invalid activeSession cursor (`currentPomodoro > totalPomodoros`) and must self-recover `running` group + `finished` session mismatches; app must reopen on correct running task/time and avoid `Pomodoro 2 of 1` / `00:00 Syncing session...`. | closed_commit_hash: `-` | closed_commit_message: `-` | evidence: Incident on 10/03 post-fix runs: activeSession had stale cursor (`currentTaskIndex=1`, `currentPomodoro=2`, `totalPomodoros=1`) and later `status=finished` while group remained `running`, producing repeated `Auto-start abort (state not idle)`. Follow-up v2 implemented in PomodoroViewModel with dedicated regression tests PASS; manual Android RMX3771 + macOS validation pending.
