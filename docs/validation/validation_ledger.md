...
- [ ] ID: `P0-F26-006` | Type: refactor | Priority: P0 | Status: In progress | Source: `docs/specs.md:2376`, `docs/roadmap.md:266`, `docs/dev_log.md` Block 577 | Item: Fix 26 rewrite contract (docs-first) — define and approve architecture invariants before tests/runtime edits: app-scope non-autoDispose `TimerService` model, exact stream-null UX thresholds, ownership stale timeout policy, and staged dual-path cutover strategy. | closed_commit_hash: `-` | closed_commit_message: `-` | evidence: Contract drafted in `docs/specs.md` section 10.4.10; roadmap updated with rewrite-branch docs-first gate. `[REWRITE-CORE]` tests blocked pending contract review approval.
