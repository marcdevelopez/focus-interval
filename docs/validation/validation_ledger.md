...
- [ ] ID: `P0-F26-006` | Type: refactor | Priority: P0 | Status: In progress | Source: `docs/specs.md:2376`, `docs/roadmap.md:266`, `docs/dev_log.md` Block 582 | Item: Fix 26 rewrite contract + Stage A runtime — migrate countdown continuity to app-scope non-autoDispose `TimerService` while preserving legacy ownership/handoff paths; stream-null hold must degrade sync only (no timer freeze). | closed_commit_hash: `-` | closed_commit_message: `-` | evidence: Contract finalized in `docs/specs.md` section 10.4.10. Runtime bridge (`248d963`) moved `[REWRITE-CORE]` to Invariants 1/2 PASS; Invariant 5 was converted from contract gate to executable continuity test and now PASS (VM dispose/rebuild). Stage B docs/test gate is now active: sections `10.4.10.8` and `10.4.10.9` were added, and Invariants 3/4 were upgraded from `fail()` placeholders to executable deterministic red tests. Current baseline: 3 PASS / 2 FAIL (pending runtime for Invariants 3/4 only). Regression smoke remains PASS on `[PHASE*]` subsets and pause/overlay suites.
