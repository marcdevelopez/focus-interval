...
- [ ] ID: `P0-F25-001` | Type: bug | Priority: P0 | Status: In validation | Source: `docs/bugs/validation_fix_2026_03_05/quick_pass_checklist.md:34` | Item: Local -> Account without false overlaps; ownership request delivered (Fix 25). | closed_commit_hash: `-` | closed_commit_message: `-` | evidence: Re-validation run #2 (2026-03-17, commit `fd788e6`) validated BUG-F25-A/B, but BUG-F25-C still failed (owner saw mirror-only modal on `Continue`). Follow-up race patch applied (pre-await `_resolved=true`) and is pending device re-validation.
...
- [ ] ID: `BUG-F25-C` | Type: bug | Priority: P1 | Status: In validation | Source: `docs/bugs/bug_log.md` (BUG-F25-C) | Item: "Owner resolved" modal shown on owner device after conflict resolution because ownerDeviceId is cleared post-resolve → isOwner evaluates false → mirror-only dialog fires. Fix: add _resolved flag in _LateStartOverlapQueueScreenState; gate condition at late_start_overlap_queue_screen.dart:176 with && !_resolved. | closed_commit_hash: `-` | closed_commit_message: `-` | evidence: Re-validation run #2 (2026-03-17, `fd788e6`) still reproduced issue on `Continue` path due race (`_resolved` set after await). Follow-up patch moved `_resolved=true` before first await in `_applySelection`; local analyze/tests PASS; pending device re-validation.
