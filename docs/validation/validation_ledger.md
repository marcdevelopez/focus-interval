...
- [ ] ID: `P0-F26-006` | Type: refactor | Priority: P0 | Status: In validation | Source: `docs/specs.md:2376`, `docs/roadmap.md:266`, `docs/dev_log.md` Block 588 | Item: Fix 26 sync rewrite execution (Stage A/B runtime complete) + Stage C stabilization — preserve timer continuity with app-scope `TimerService`, keep `SessionSyncService` as remote-ingestion authority, and close Stage C findings: `O-1` terminal-boundary hold suppression and `O-2` ref-after-dispose hardening in delayed recovery callbacks. | closed_commit_hash: `-` | closed_commit_message: `-` | evidence: Stage C pass1 logs (`2026-03-14_fix26_rewrite_stageC_c0add32_pass1_1h_*`) reviewed and accepted for P0 vectors (AP-1/AP-2 not reproduced). Codex follow-up implementation added in `session_sync_service.dart` + `pomodoro_view_model.dart` with new regressions in `pomodoro_view_model_session_gap_test.dart`; local `flutter analyze` PASS and targeted suite PASS. Final closure blocked until Stage C pass2 soak logs (`2026-03-16_fix26_rewrite_stageC_c0add32_pass2_4h_{android,macos}_debug.log`) are analyzed.
