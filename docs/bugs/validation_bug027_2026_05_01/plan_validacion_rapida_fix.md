...
### Non-authoritative for this bug

- UI strings such as `Running:`, `Scheduled:`, `Pre-Run:`, and the base mirror warning copy are rendered in widgets and are not expected to be reliably present in console logs.
- Treat missing UI-copy grep hits as inconclusive; use visual capture on each surface as the pass/fail source.
...
- `flutter test test/presentation/viewmodels/pomodoro_view_model_session_gap_test.dart` -> PASS (2026-05-01, clean re-run).
- `flutter test test/presentation/viewmodels/pomodoro_view_model_pause_expiry_test.dart` -> PASS (2026-05-01).
