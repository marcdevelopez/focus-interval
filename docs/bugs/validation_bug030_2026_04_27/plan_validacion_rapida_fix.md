...
still on the timer route. It fired equally on every intentional departure.
...
On macOS, `AppLifecycleState.resumed` fired on each window focus change. It
cleared suppression on every window-focus event.

### Cause 3 — Bounce window (tertiary)

`lib/widgets/active_session_auto_opener.dart` lines 148-157: rarely contributed
independently because cause 1 already cleared `_autoOpenedGroupId` first.

**Critical ordering constraint:** causes 1 and 2 run before the navigator context
block. Departure-detection flags must be set before those paths execute.
...
2. Remain in Groups Hub for at least 60 seconds without manually returning to Run Mode.
...
2. Enter Plan Group (`/tasks/plan`) and remain there for at least 60 seconds.
...
Steps:
1. Keep mirror on Run Mode during a complete pomodoro phase transition.
...
Any match on this line while user is in `/groups` or `/tasks` means BUG-030 is still active.
